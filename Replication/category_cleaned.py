...
class Category:
    ...
    def check_probs(self):
        ...
        print(f"Category: {self._label}")
        print(f"Seen probabilities: {total}")
        unseen = meaning._unseen * (self._meaning_normalization - len(features))
        print(f"Unseen probability: {unseen}")
        total += unseen 
        print(f"Total: {total}")
    ...
    def print_label(self, id):
        ...
        for l in category_labels.keys():
            if category_labels[l] > max:
                ...
                label = l
        if label == "":
            print(f"No label found for category with ID: {id}")
            return
        
        print(f"Label: {label}")
        print(f"Precision: {float(category_labels[label] / sum(category_labels.values()))}")
    def print_category(self, id):
        """ Print detailed category information. """
        category = self._categories[id]
        print(f"{id} ({len(category._words.keys())}): {{")
        for word in category._words.keys(): 
            print(word, ":", category._words[word], ", ",)
# [REMOVED INVALID PRINT] original line was: print( }\n)
...
def semantic_clustering_categories(beta,  words, lexicon, features, wnlabels,  stopwords, pos=CONST.ALL, 
    ...
    print(f"Number of words in Clustering: {len(filtered_words)}")
...
def form_wordnet_categories(words, wnlabels):
    ...
    print(f"Number of words without WN categories: {errors}")
